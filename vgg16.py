...
x_test = np.zeros(shape=(733, 25088))  # shape is (num images,400*300*3)
y_test = []
...
if os.path.getsize('vectors2.tsv') == 0:
    with open('vectors2.tsv', 'w') as vectors:
        for row in df.iterrows():
            print(index)
            array = row[1].array
            array = array[0].split(',')
            ...
            vec = np.char.mod('%f', features.flatten())
